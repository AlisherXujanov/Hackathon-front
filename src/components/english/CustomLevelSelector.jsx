  ...
  return (
    <div className={`relative ${className}`} ref={dropdownRef}>
      <motion.button
        ...
        className={`flex items-center gap-2 bg-white/95 backdrop-blur-md rounded-lg px-3 py-2 text-sm font-medium text-gray-700 border border-white/30 hover:bg-white hover:border-primary-300/50 transition-all duration-200 focus:outline-none focus:ring-2 focus:ring-primary-500/50 focus:ring-offset-2 focus:ring-offset-transparent shadow-lg hover:shadow-xl min-w-[140px] group ${
          compact ? 'text-xs px-2.5 py-1.5' : ''
        }`}
        whileHover={{ scale: 1.02 }}
        whileTap={{ scale: 0.98 }}
      >
        <HiAcademicCap className={`text-gray-600 group-hover:text-primary-600 transition-colors duration-200 ${compact ? 'w-3.5 h-3.5' : 'w-4 h-4'}`} />
        ...
        >
          <HiChevronDown className={`text-gray-500 group-hover:text-primary-600 transition-colors duration-200 ${compact ? 'w-3.5 h-3.5' : 'w-4 h-4'}`} />
        </motion.div>
      </motion.button>

      {/* Animated Dropdown Menu with Slide-in */}
      <AnimatePresence>
        {isOpen && (
          <motion.div
            ...
            animate="visible"
            exit="exit"
            className="absolute top-full right-0 mt-2 bg-white/98 backdrop-blur-md rounded-xl border border-gray-200/50 shadow-2xl z-50 min-w-[200px] overflow-hidden"
            style={{
